package pl.komjago.foreigncurrencyaccounts.controllers.v1.accounts

import org.springframework.http.HttpStatus
import org.springframework.validation.annotation.Validated
import org.springframework.web.bind.annotation.GetMapping
import org.springframework.web.bind.annotation.PathVariable
import org.springframework.web.bind.annotation.PutMapping
import org.springframework.web.bind.annotation.RequestBody
import org.springframework.web.bind.annotation.RequestMapping
import org.springframework.web.bind.annotation.ResponseStatus
import org.springframework.web.bind.annotation.RestController
import pl.komjago.foreigncurrencyaccounts.controllers.v1.accounts.dto.AccountInfo
import pl.komjago.foreigncurrencyaccounts.controllers.v1.accounts.dto.RegisterAccountInput
import pl.komjago.foreigncurrencyaccounts.services.accounts.AccountsService
import pl.komjago.foreigncurrencyaccounts.utils.validation.PESEL
import javax.validation.Valid

@RestController
@Validated
@RequestMapping("/api/v1/accounts")
class ForeignCurrencyAccountsController(
    private val accountsService: AccountsService
) {

    @GetMapping("/{pesel}")
    fun get(@PathVariable @PESEL pesel: String): AccountInfo {
        return accountsService.get(pesel)
    }

    @ResponseStatus(HttpStatus.CREATED)
    @PutMapping("/register")
    fun register(@RequestBody @Valid registerAccountInput: RegisterAccountInput) {
        return accountsService.register(registerAccountInput)
    }
}